= Acts As Tenant
This gem is based on the Multitenant gem by Ryan Sonnek and some of his code to set the default_scope. This gem was born out of the need for a fail-safe and out-of-the-way manner to add multi-tenancy to a Rails app, that integrates seamlessly with Rails.
...
Adding acts_as_tenant to your model will scope that model to the current tenant **if a current tenant has been set**. Acts_As_Tenant requires each scoped model to have a column in its db schema identifying its tenant. So in this example the migration for the Project model might look as follows:


  @project = Project.new(:name => 'big project')    # 



Setting the current_tenant is handled using the Acts_As_Tenant's methods in application controller.
